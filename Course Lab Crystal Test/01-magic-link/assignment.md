...
instead of vensim. To check the deployment's status directly against
Crystal's own API, run the below in the **cloud console** tab:

```run
curl -s -H "X-API-Key: $CRYSTAL_API_KEY" "$(agent variable get CRYSTALBASEURL)/api/deployments/$(agent variable get CRYSTALDEPLOYMENT)"
```

> [!NOTE]
> The deployment is created and started once, during sandbox setup -
> it then runs on Project Crystal's own infrastructure, not as a local
> background process on this container, so there's no local log file
> to tail the way vensim had one.
